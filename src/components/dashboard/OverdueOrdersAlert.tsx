      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex gap-2">
          {severeDelays.length > 0 && (
            <Badge className={getDelayColor('severe')}>
              {severeDelays.length} Crítica{severeDelays.length > 1 ? 's' : ''}
            </Badge>
          )}
          {moderateDelays.length > 0 && (
            <Badge className={getDelayColor('moderate')}>
              {moderateDelays.length} Moderada{moderateDelays.length > 1 ? 's' : ''}
            </Badge>
          )}
          {mildDelays.length > 0 && (
            <Badge className={getDelayColor('mild')}>
              {mildDelays.length} Leve{mildDelays.length > 1 ? 's' : ''}
            </Badge>
          )}
        </div>
        ...
        <div className="p-3 bg-white border border-red-200 rounded-lg">
          ...
          <div className="flex gap-2 mt-3">
            {onNavigate && mostUrgentOrder.pickupAddress && (
              <Button
                onClick={() => onNavigate(mostUrgentOrder.pickupAddress!)}
                size="sm"
          ...
          <div className="space-y-2">
            ...
            {overdueOrders.slice(1).map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="p-2 bg-white border border-red-100 rounded cursor-pointer hover:bg-red-50"
