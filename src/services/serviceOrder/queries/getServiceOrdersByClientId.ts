...
export async function getServiceOrdersByClientId(clientId: string, includeArchived: boolean = false): Promise<ServiceOrder[]> {
  try {
    let query = supabase
      .from('service_orders')
      .select('*')
    ...
    console.error(`Erro ao buscar ordens de serviço para o cliente ${clientId}:`, error);
    return [];
  }
}
