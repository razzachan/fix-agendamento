...
WHERE NOT EXISTS (
  ...
  WHERE ce.service_order_id = s.service_order_id AND s.service_order_id IS NOT NULL
);
...
CREATE POLICY "Admins têm acesso total" ON calendar_events
  FOR ALL
  TO authenticated
  USING (
    EXISTS (
...
-- Política para técnicos (apenas seus próprios eventos)
CREATE POLICY "Técnicos veem apenas seus eventos" ON calendar_events
  FOR SELECT
  TO authenticated
  USING (
    (
      ...
      EXISTS (
        ...
        WHERE t.user_id = auth.uid() AND t.id = calendar_events.technician_id
      )
    )
  );
