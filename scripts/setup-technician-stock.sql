-- =====================================================
-- SCRIPT DE SETUP: SISTEMA DE ESTOQUE MÓVEL
...
-- 9. ESTOQUE INICIAL PARA TÉCNICO PEDRO SANTOS
-- (Substitua o UUID pelo ID real do técnico)
DO $$
DECLARE
    pedro_id UUID;
    item_record RECORD;
BEGIN
    -- Buscar ID do Pedro Santos
    SELECT id INTO pedro_id FROM auth.users WHERE email LIKE '%pedro%' OR raw_user_meta_data->>'name' LIKE '%Pedro%' LIMIT 1;
    ...
    ELSE
        RAISE NOTICE 'Técnico Pedro Santos não encontrado. Execute manualmente após criar o usuário.';
    END IF;
END $$;
